          ...
          <div key={gen} className="space-y-2">
            ...
            {activeLines.length === 0 ? (
              <div className="flex justify-center py-3">
                <p className="text-[10px] font-sans text-stone-400 italic">No records in this generation</p>
              </div>
            ) : (
            <div className="grid gap-1 px-1" style={{ gridTemplateColumns: `repeat(${activeLines.length}, 1fr)` }}>
            ...
            {gi < gens.length - 1 && activeLines.length > 0 && (
              <div className="grid gap-1 px-1 mt-1" style={{ gridTemplateColumns: `repeat(${activeLines.length}, 1fr)` }}>
                {activeLines.map((line, li) => {
                  const hasInNext = (byGenAndLine[gens[gi + 1]]?.[line]?.length ?? 0) > 0;
                  return (
                    <div key={line} className={`flex justify-center ${li > 0 ? 'border-l border-transparent pl-2' : ''}`}>
                      <div className={`w-px h-3 border-r border-dashed ${hasInNext ? 'border-stone-400' : 'border-stone-300'}`} />
                    </div>
                  );
                })}
              </div>
